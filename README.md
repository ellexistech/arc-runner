...
image with **Node 24 LTS**, **pnpm**, `gh`, and `jq` (static/official binaries,
no apt python3) so workflow setup steps can skip downloading those tools on
every ephemeral pod.
...
| Tag                                     | Meaning                                     |
| --------------------------------------- | ------------------------------------------- |
| `ghcr.io/ellexistech/arc-runner:0.2.0`  | Immutable release (pin this in Helm values) |
| `ghcr.io/ellexistech/arc-runner:latest` | Same build, moving pointer                  |


### Release a new image

1. Edit `VERSION` (e.g. `0.2.0` → `0.3.0`).
